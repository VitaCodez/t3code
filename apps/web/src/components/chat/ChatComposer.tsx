...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const submitComposer = useCallback(
    (event?: { preventDefault: () => void }, intent: ComposerSubmissionIntent = "foreground") => {
      ...
      // in the *next* draft instead. Only oversized images hit this â€” small
  ...
  const stashCurrentPrompt = useCallback(async () => {
    ...
    // A repeat âŚS on the *same* still-unencoded snapshot would stash it
    ...
    try {
      ...
      if (attached) {
        ...
        // which reads as an orphan after reload â€” so say so now. Gated on the
      ...
      // snapshot's âŚS until the composer remounts.
    ...
    if (acceptedFiles.length > 0) {
      ...
      if (options?.source?._tag === "pasted-text" && storedFiles.length > 0) {
        ...
        toastManager.add({
          ...
          description: `${formatAttachmentSize(attached.sizeBytes)} Â· Use ${
            isMacPlatform(navigator.platform) ? "âŚâ‡§V" : "Ctrl+Shift+V"
