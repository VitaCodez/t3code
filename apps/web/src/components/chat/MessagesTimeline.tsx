...
// ---------------------------------------------------------------------------
// Context Ă˘â‚¬â€ť shared state consumed by every row component via Context.
// Propagates through LegendList's memo boundaries for shared callbacks and
// non-row-scoped state. `nowIso` is intentionally excluded Ă˘â‚¬â€ť self-ticking
  ...
  return (
    <div className={fade ? "pt-[var(--workspace-titlebar-scroll-fade-height)]" : "pt-3 sm:pt-4"}>
      <div className="mx-auto w-full max-w-3xl pb-2">
        ...
        >
          {loading ? "Loading earlier turnsĂ˘â‚¬Â¦" : "Load earlier turns"}
        </button>
      </div>
    </div>
  );
...
interface MessagesTimelineProps {
   ...
   * projection and list extraData Ă˘â‚¬â€ť do not remount on this value.
   */
...
// ---------------------------------------------------------------------------
// MessagesTimeline Ă˘â‚¬â€ť list owner
// ---------------------------------------------------------------------------
...
export const MessagesTimeline = memo(function MessagesTimeline({
  ...
  // Stable renderItem Ă˘â‚¬â€ť no closure deps. Row components read shared state
...
// ---------------------------------------------------------------------------
// TimelineRowContent Ă˘â‚¬â€ť the actual row component
// ---------------------------------------------------------------------------
...
/**
 * Hover-revealed wall-clock time with a full-date tooltip Ă˘â‚¬â€ť the same metadata
...
function ActivityGroupTimelineRow({
  ...
  const label = row.active
    ...
    : work.length > 0
      ...
      : `Thought${thoughtCount > 1 ? ` (Ä‚â€”${thoughtCount})` : ""}`;
...
// ---------------------------------------------------------------------------
// Self-ticking labels Ă˘â‚¬â€ť update their own text nodes so elapsed-time display
...
// ---------------------------------------------------------------------------
// Extracted row sections Ă˘â‚¬â€ť own their state / store subscriptions so changes
 ...
 *  so toggling re-renders only this component Ă˘â‚¬â€ť not the entire list. */
    ...
    <div className="space-y-2 rounded-lg border border-border/70 bg-background/70 p-3">
      <div className="space-y-1">
        ...
        <div className="text-secondary-label text-[11px]">
          {comment.sectionTitle} Ă‚Â· {comment.rangeLabel}
        </div>
      </div>
...
// ---------------------------------------------------------------------------
// Structural sharing Ă˘â‚¬â€ť reuse old row references when data hasn't changed
...
const AgentSpawnRow = memo(function AgentSpawnRow(props: {
  ...
  return (
    <div className="flex flex-col">
      ...
      >
        <LiveActivityRow
          label={workflowName ? `${lead} Ă‚Â· ${workflowName}` : lead}
          iconName="bot"
...
function AgentSpawnMemberRow({
  ...
  const activity = activeStatus
    ? (agent.progress ?? (agent.lastToolName ? `Ă˘â€“Â¸ ${agent.lastToolName}` : null))
  ...
  const statusLabel =
    activeStatus || !meta
      ...
      : agent.status === "completed"
        ? meta
        : `${AGENT_MEMBER_STATUS_LABEL[agent.status]} Ă‚Â· ${meta}`;
  const role =
